...
# --- type it --------------------------------------------------------------
# One compound input: BEGIN marker; { CMD }; DONE marker with rc.
# Markers are assembled by printf at *execution* time so the echoed
# keystrokes never contain the literal marker strings — extraction can't
# match the echo. Everything between BEGIN and DONE output is pure command
# output. Leading space keeps it out of shell history (hist_ignore_space).
...
# Slice between the markers, strip ANSI/OSC sequences and CRs for the
# agent; the human pane already saw the colorful version.
tr -d '\r' <"$LOG" | awk -v id="$ID" '
  index($0, "<<AGENT-DONE:" id ":") { exit }
  on { print }
  index($0, "<<AGENT-BEGIN:" id ">>") { on = 1 }
