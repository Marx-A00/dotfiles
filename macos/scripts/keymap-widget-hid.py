...
Opens the hotdox76v2's raw HID interface (usage page 0xFF60 / usage 0x61)
and relays the firmware's layer broadcasts ([0x4C, layer, 0...]) to stdout
as "L<n>" lines. Hammerspoon (keymap-widget.lua) spawns and supervises this
with hs.task and forwards each line to the hotdox webview's __setLayer(n).
...
TAG = 0x4C  # 'L' — layer frames from keymaps/mrx/keymap.c; drop all others
