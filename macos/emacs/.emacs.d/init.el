...
# Clear your mind young one.")
  ...
  (defun mr-x/fetch--center ()
    "Center the *fetch* content in its window.
Horizontal via a buffer-local `line-prefix', vertical via blank
lines above the content.  Registered on a buffer-local
`window-configuration-change-hook', so it re-centers whenever the
buffer is displayed or the window is resized."
  ...
  (defun mr-x/fetch--compose ()
    "Logo and info side by side as one raw ANSI string.
fastfetch normally does this layout with cursor-movement escapes,
which ansi-color strips rather than interprets — so run it twice
(logo only, info only) and do the column math here.  The logo's SGR
color state spans lines, so carry it across the interleaved info
text and reset at each line end."
    (let* ((logo-raw (replace-regexp-in-string
                      "\e\\[[0-9;]*[ABCDG]" ""
                      (mr-x/fetch--run "--structure" "break")))
           (info-raw (mr-x/fetch--run
                      "--logo" "none" "--structure"
                      "title:separator:os:kernel:uptime:packages:shell:cpu:memory"))
           (trim "\\(?:\e\\[[0-9;]*m\\|[ \t\n]\\)+\\'")
           (logo-lines (split-string
                        (replace-regexp-in-string trim "" logo-raw) "\n"))
           (info-lines (split-string
                        (replace-regexp-in-string trim "" info-raw) "\n"))
           (logo-width (apply #'max (mapcar #'mr-x/fetch--visible-width
                                            logo-lines)))
           (gap 3)
           (state "")
           out)
      (dotimes (i (max (length logo-lines) (length info-lines)))
        (let ((ll (or (nth i logo-lines) ""))
              (il (or (nth i info-lines) ""))
              (pos 0))
      ...
      (with-current-buffer buf
        (let ((inhibit-read-only t))
          ...
          (if (executable-find "fastfetch")
              (progn
                (require 'ansi-color)
                (insert (mr-x/fetch--compose))
                (ansi-color-apply-on-region (point-min) (point-max)))
            (insert (format "%s@%s · Emacs %s\n"
                            (user-login-name) (system-name) emacs-version)))
          (goto-char (point-min)))
        (special-mode)
        ;; special-mode kills local variables/hooks — register after it.
        (add-hook 'window-configuration-change-hook #'mr-x/fetch--center nil t))
      buf))
