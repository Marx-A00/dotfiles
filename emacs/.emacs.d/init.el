...
(use-package shackle
  ...
  :config
  (setq shackle-rules
        '(;; Agent Shell windows (side panel like an IDE)
          (".*Agent @.*"
           :regexp t
           :align left
           :size 0.30
...
(use-package agent-shell
  ...
  ;; Fix agent-shell-toggle to find project shells when not in a shell buffer
  ;; Upstream bug: agent-shell-toggle uses agent-shell--current-shell which
  ;; only works when already in a shell/viewport buffer
