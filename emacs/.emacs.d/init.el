...
Highlights the actual code content, not just +/- markers."
    ...
    ;; Fix: Override 'q' in agent-shell diff buffers to exit cleanly
    ;; Prevents the "Accept changes? y or n" prompt loop
    (defun mr-x/agent-shell-diff-clean-exit ()
      "Exit diff buffer cleanly without triggering on-exit prompt."
      (interactive)
      (setq agent-shell-on-exit nil)
      (kill-current-buffer))
    
    ;; Apply clean exit to all agent-shell diff buffers
