...
#+begin_src emacs-lisp
  ...
  :custom
  (persp-show-modestring nil)  ; hide perspective name from modeline (doom-modeline handles it)
  (persp-mode-prefix-key (kbd "C-x M-x"))  ; keep original prefix for compatibility
  :init
  (persp-mode)
  :config
  ;; persp-mode adds itself to global-mode-string during activation and
  ;; ignores persp-show-modestring afterwards — remove it explicitly
