            ...
            (mapcar (lambda (f)
                      (list :left (frame-parameter f 'left)
                            ...
                            :buffers (mapcar (lambda (w)
                                            ...
                                            (window-list f 'no-mini))
                            :window-config (window-state-get (frame-root-window f))))
                    frames)))
      ...
      (let ((frame-data (with-temp-buffer
        ...
        (dolist (fd frame-data)
          (let ((frame (make-frame
                        (list (cons 'left (plist-get fd :left))
            ...
            ;; Restore files/buffers first so they exist
            (dolist (buf-info (plist-get fd :buffers))
              (let ((file (plist-get buf-info :file))
                    (name (plist-get buf-info :buffer)))
                (when (and file (file-exists-p file))
                  (find-file-noselect file))))
            ;; Restore window configuration
            (condition-case nil
                (window-state-put (plist-get fd :window-config)
                                  (frame-root-window frame)
                                  'safe)
              (error
               ;; Fallback: just show the first buffer
               (let* ((first-buf (car (plist-get fd :buffers)))
                      (file (plist-get first-buf :file))
                      (name (plist-get first-buf :buffer)))
                 (when file
                   (select-frame frame)
                   (find-file file)))))))
        (message "Session restored: %d frames" (length frame-data)))))
